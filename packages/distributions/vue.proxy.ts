...
export const PlusBrowse = /*@__PURE__*/ proxy<
  HTMLPlusBrowseElement,
  JSX.PlusBrowse
>("plus-browse", {
  prefix: "plus",
  props: [
...
const mergeClasses = (
  ...
  return [...Array.from(ref.value?.classList || []), ...defaultClasses].filter(
    (item, index, items) =>
      !componentClasses.has(item) && items.indexOf(item) === index
  );
};
...
export const proxy = <ElementType, PropType>(
  ...
  const { prefix, props = [], model } = options || {};
  ...
  const VueComponent = defineComponent<PropType & InputPropsType>(
    (props, context) => {
      ...
      return () => {
        ...
        if (model) {
          ...
          newProps[model.prop] =
            props.hasOwnProperty(MODEL_PROP) && props[MODEL_PROP] !== undefined
              ? props.modelValue
              : modelValue;
        }
