...
module.exports = ({ ast, generator }) => {
    ...
    const options = {
        template: {
            ClassDeclaration(path) {
                ...
                path.traverse(options.template);
            ...
            JSXAttribute(path) {
                ...
                if (name.name.match(/on[A-Z]/)) {
                    ...
                    name.name = '@' + utils.toCamelCase(name.name.replace('on', ''));
                }
    ...
    return {
        template: generator(t.cloneNode(ast), options.template).trim().slice(0, -1).replace(/\[\[/g, '{{').replace(/\]\]/g, '}}'), // TODO
        script: generator(t.cloneNode(ast), options.script),
    }
};